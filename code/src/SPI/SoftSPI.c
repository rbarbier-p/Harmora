...
{
    // Clock pin setup
    spi->clk_ddr  = softspi_pinmap[clk_pin].ddr;
    spi->clk_port = softspi_pinmap[clk_pin].port;
    spi->clk_bit  = softspi_pinmap[clk_pin].bit;

    // MOSI pin setup
    spi->mosi_ddr  = softspi_pinmap[mosi_pin].ddr;
    spi->mosi_port = softspi_pinmap[mosi_pin].port;
    spi->mosi_bit  = softspi_pinmap[mosi_pin].bit;

    // MISO pin setup (optional)
    if (miso_pin != 0xFF && miso_pin < sizeof(softspi_pinmap)/sizeof(softspi_pinmap[0])) {
        spi->miso_ddr  = softspi_pinmap[miso_pin].ddr;
        spi->miso_pin  = softspi_pinmap[miso_pin].pin;
        spi->miso_bit  = softspi_pinmap[miso_pin].bit;
        spi->miso_enabled = 1;
